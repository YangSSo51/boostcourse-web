...
1. 클라이언트가 서버에 접속 (C__onnect__)
2. 클라이언트가 서버에게 요청 (__Request__)
    - 요청 헤더 : 요청 메소드/요청 URI/HTTP 프로토콜 버전
3. 서버가 클라이언트에게 응답 (__Response__)
...
#### 역할
- 웹 콘텐츠를 잘 보여주기 위한 __구조__ (__HTML__)
- 적절한 배치와 __디자인__ 제공 (__CSS__)
- 사용자 요청을 잘 반영하여 __동작__ 해야함 (__JavaScript__)
### 웹 Back-End
...
- HTML은 계층적이고 __tag__ 를 사용해서 표현함  
...
# 1-6. 웹서버
### 웹서버
- 웹서버 소프트웨어 혹은 웹 서버 소프트웨어가 동작하는 컴퓨터
- 클라이언트가 요청하는 HTML문서나 각종 리소스를 전달
- 웹브라우저나 웹 크롤러가 요청하는 리소스는 컴퓨터에 저장되어있는 정적인 데이터거나 동적인 결과가 될 수 있음
- 정적인 데이터(이미지,html,css,js)
- 동적인 결과(프로그램을 통해 만들어진 결과)

#### 종류
- Apache,Nginx,Microsoft,Google
...
- 데이터베이스 관리 시스템
### 미들웨어
- 클라이언트와 DBMS사이에 다른 서버를 두는 것
- 클라이언트의 로직을 미들웨어에서 처리해줌
### WAS
- 브라우저와 DBMS 사이에서 동작하는 미들웨어
- 프로그램 실행 환경,데이터베이스 접속 기능을 제공
- 여러개의 트랜잭션을 관리
...
웹서버 | WAS 
:---:|:---:
정적인 콘텐츠를 웹브라우저에게 전송 | 동적인 결과를 웹 브라우저에게 전송 
상대적으로 간단한 구조 | 문제가 생기면 재시작해야함 

* 웹서버가 WAS 앞단에 있으면 좋은 이유
 - 대용량 어플리케이션의 경우 오작동이 발생했을 때 장애 극복 기능이 필요
 - 앞 단의 웹서버에서 문제가 발생한 WAS를 이용하지못하게 하고 WAS를 재시작시켜서 문제를 노출시키지않음

### 생각해보기
1. 톰캣 버전별 차이점에 대해서 알아보세요.  
tomcat 버전 별로 지원하는 JAVA 버전, 서블릿, 웹소켓, ELS 등이 다름

학습 출처 : [BoostCourse](https://www.edwith.org/boostcourse-web/joinLectures/12952)
